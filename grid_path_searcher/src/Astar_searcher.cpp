    ...
    for (int i = x - 1; i < x + 2; i++)
    {
        for (int j = y - 1; j < y + 2; j++)
        {
            for (int k = z - 1; k < z + 2; k++)
            {
                ...
                if (isOccupied(i, j, k) != 1 && GridNodeMap[i][j][k]->id != -1)
                {
                    GridNodePtr tempPtr = GridNodeMap[i][j][k];
                    neighborPtrSets.push_back(tempPtr);
                    Eigen::Vector3d this_coord = currentPtr->coord;
                    Eigen::Vector3d n_coord = tempPtr->coord;
                    double dis = std::sqrt((n_coord[0] - this_coord[0]) * (n_coord[0] - this_coord[0]) +
...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2)
{
    ...
    */
    ...
    for (int i = 0; i < 3; i++)
    {
        res += (node1->coord[i] - node2->coord[i]) * (node1->coord[i] - node2->coord[i]);
    }
    res = sqrt(res);
    return res;
}
    ...
    */
    ...
    while ( !openSet.empty() ){
        ...
        *
        ...
        please write your code below
        *      currentPtr->id=-1;    currentPtr->id=-1;
        */         
